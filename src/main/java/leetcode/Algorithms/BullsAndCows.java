 ...
 *
 * The number of "cows", which are digits in the guess that
 * are in your secret number but are located in the wrong
 * position. Specifically, the non-bull digits in the guess
 * that could be rearranged such that they become bulls.
 *
 * Given the secret number secret and your friend's
 * guess, return the hint for your friend's guess.
 *
 * The hint should be formatted as "xAyB", where x is the
 * number of bulls and y is the number of cows. Note that
 ...
 *   |      or     |
 * "0111"        "0111"
 * Note that only one of the two unmatched 1s is counted as
 * a cow since the non-bull digits can only be rearranged
