...
public class DecodeString {
    public String decodeString(String s) {
        ...
        for  (int i = 0; i < s.length(); i++) {
            char ithCh = s.charAt(i);
            if (ithCh != ']') {
                stack.add(Character.toString(ithCh));
                continue;
            }
            ...
            // one repetition is already in sb.
            top = "";
            while (!stack.isEmpty() && stack.peek().charAt(0) - '0' < 10 ) {
                top = stack.pop() + top;
            }
            int repeat = Integer.parseInt(top) - 1;
